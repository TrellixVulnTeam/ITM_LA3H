...
def import_data(conn,AGENT_NAME,AGENT_CODE,AGENT_VERSION,HOST_NAME,IP_ADDRESS,INSTANCE,AGENT_HOST,AGENT_TYPE):
    global counter
    sqlStr = "insert into " + TABLE_NAME + " (AGENT_NAME,AGENT_CODE,AGENT_VERSION,HOST_NAME,IP_ADDRESS,INSTANCE,AGENT_HOST,AGENT_TYPE)" \
             "values (?,?,?,?,?,?,?,?) "
    conn.execute(sqlStr,(AGENT_NAME,AGENT_CODE,AGENT_VERSION,HOST_NAME,IP_ADDRESS,INSTANCE,AGENT_HOST,AGENT_TYPE))
    conn.commit()
    counter += 1
        ...
        for line in file_to_read.readlines():
            ...
            if len(line_tmp) == 6:  #[93]  ip.pipe:#10.1.8.178[22399]<NM>P74017P1</NM>  iftsdb:P74017P1:RZ  RZ  06.31.02  AIX~6.1
                ...
                if agent_code == 'RZ':  #RZ:OIBS1-OIBS1-BL660216:RDB
                    ...
                    try:
                        ip_address = host_dict[hostname.upper()]
                    except:
                        ip_address = ''
